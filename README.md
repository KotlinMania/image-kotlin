...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`image`](https://crates.io/crates/image).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `image` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Multi-format image codec (PNG, JPEG, GIF, WebP, ...)
...
Original work copyrighted by the upstream `image` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
