# Claude Code Project Instructions — image-kotlin

## Project Overview

This is **image-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`image`](https://crates.io/crates/image).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/image/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/image>
